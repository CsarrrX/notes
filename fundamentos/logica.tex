...
A partir del condicional simple, podemos derivar dos estructuras que no añaden conceptos nuevos, sino que reinterpretan la relación de implicación:
\begin{flalign*}
    &\text{1. \textbf{Bicondicional:}} \quad p \iff q \quad \equiv \quad (p \implies q) \land (q \implies p) &
\end{flalign*}
Decimos que $p$ y $q$ son \textit{lógicamente equivalentes}. Esto significa que ambos enunciados están <<atados>>: si uno es verdadero, el otro obligatoriamente lo es; y si uno es falso, el otro también.
\begin{flalign*}
    &\text{2. \textbf{Contrapositiva:}} \quad \neg q \implies \neg p \quad \equiv \quad p \implies q &
\end{flalign*}
Esta es simplemente una \textit{re-escritura} de la implicación. En matemáticas, la contrapositiva es la <<puerta trasera>> de las demostraciones: si no puedes probar que $p$ conduce a $q$, puedes probar que la ausencia de $q$ implica la ausencia de $p$. Ambas afirmaciones contienen exactamente la misma información lógica.
