...
El orden es irrelevante si los cuantificadores son del mismo tipo:
\begin{itemize}
...
Si se combinan $\forall$ y $\exists$, el orden es crítico porque define la \textbf{dependencia} de las variables. Sea el dominio $\mathbb{Z}$:
\begin{itemize}
    \item $\forall n \exists m (m > n)$: \textbf{Verdadero}. Para cada $n$, existe un $m$ que puede depender de $n$ (ej. $m = n + 1$).
    \item $\exists m \forall n (m > n)$: \textbf{Falso}. Implicaría la existencia de un entero $m$ fijo que es mayor que todos los demás.
\end{itemize}
\end{ejemplo}
